...
# The __main__ guard is required, not stylistic: the Clarke-Wright initialization
# uses multiprocessing.Pool, and on platforms whose default start method is
# `spawn` (macOS, Windows) each worker re-imports this module. Without the guard
# every worker re-runs the whole evaluation and spawns its own pool.
